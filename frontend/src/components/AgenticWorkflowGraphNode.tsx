...
// Compact per-kind icons — deliberately plain geometric glyphs (not a full
// icon library) so every node reads at a glance without needing the label
// text, matching how Camunda/n8n/UiPath keep on-canvas nodes minimal and
// push detail into a side panel (see AgenticNodeDetailPanel.tsx) instead of
// cramming it onto the node itself.
...
export default function AgenticWorkflowGraphNode({ data }: NodeProps<AgenticWorkflowGraphNodeData>) {
  ...
  if (node.node_kind === 'gateway') {
    // Diamond, per BPMN gateway convention (Camunda renders gateways the
    // same way) — visually distinct from every task/service/human node at a
    // glance, not just a different fill color.
    return (
  ...
  return (
    <div
      ...
      className={[
        ...
        // citation summary on the node itself (all moved to the click-through
        // detail panel); this is what actually kills the scrolling problem,
        // not just tighter layout spacing (see layout.ts).
